...
function getEffectiveRole(privateState, uid) {
  const baseRole = String(privateState?.[uid]?.role || '');
  if (baseRole !== 'student') return baseRole;
  return String(privateState?.[uid]?.copiedRole || baseRole);
}
...
export function checkWinner(publicState, privateState) {
  ...
  const aliveRoles = alive.map((p) => getEffectiveRole(privateState, p.uid) || '');
  if (aliveRoles.some((role) => !role)) return '';
  const pobAlive = alive.filter((p) => getEffectiveRole(privateState, p.uid) === 'pob').length;
...
export function resolveNight(publicState, privateState) {
  ...
  const alive = Object.values(pub.players || {}).filter((p) => p.alive);
  const uidByRole = (role) => alive.filter((p) => getEffectiveRole(priv, p.uid) === role).map((p) => p.uid);
  ...
  const doctorUsedByUid = {};
  const doctorUids = uidByRole('doctor');
  doctorUids.forEach((doctorUid) => {
    const alreadyUsed = Boolean(priv?.[doctorUid]?.doctorUsed);
    if (alreadyUsed) {
      roleResults[doctorUid]?.push('คุณใช้สกิลชุบชีวิตไปแล้ว');
      return;
    }
    const doctorAction = getNightActionForDay(priv, doctorUid, day);
    const targetId = String(doctorAction?.targetId || '').trim();
    if (!targetId) return;
    if (isBlockedByPolice(priv, jailedTonight, doctorUid, day)) {
      roleResults[doctorUid]?.push('คุณโดนขัง');
      return;
    }
    const target = pub.players?.[targetId];
    if (!target || target.alive) {
      roleResults[doctorUid]?.push('เป้าหมายที่เลือกยังไม่ตาย จึงชุบไม่ได้');
      return;
    }
    target.alive = true;
    target.deathCause = '';
    doctorUsedByUid[doctorUid] = true;
    logs.push(`${target.name || 'ผู้เล่น'} ถูกหมอชุบชีวิต`);
    roleResults[doctorUid]?.push(`คุณชุบชีวิต ${target.name || 'ผู้เล่น'} สำเร็จ`);
    roleResults[targetId]?.push('คุณถูกชุบชีวิต คืนนี้ยังใช้ action ไม่ได้');
  });

  const studentRoleByUid = {};
  const studentUids = uidByRole('student');
  studentUids.forEach((studentUid) => {
    const alreadyCopiedRole = String(priv?.[studentUid]?.copiedRole || '').trim();
    if (alreadyCopiedRole) {
      roleResults[studentUid]?.push(`คุณคัดลอกบท ${alreadyCopiedRole} ไปแล้ว`);
      return;
    }
    const studentAction = getNightActionForDay(priv, studentUid, day);
    const targetId = String(studentAction?.targetId || '').trim();
    if (!targetId) return;
    if (isBlockedByPolice(priv, jailedTonight, studentUid, day)) {
      roleResults[studentUid]?.push('คุณโดนขัง');
      return;
    }
    if (jailedTonight[targetId]) {
      roleResults[studentUid]?.push(`เป้าหมาย ${pub.players?.[targetId]?.name || 'ผู้เล่น'} โดนขัง จึงติดตามไม่สำเร็จ`);
      return;
    }
    if (!pub.players?.[targetId]?.alive) {
      roleResults[studentUid]?.push('เป้าหมายตายแล้ว จึงติดตามไม่ได้');
      return;
    }
    const copiedRole = getEffectiveRole(priv, targetId);
    if (!copiedRole || copiedRole === 'student') return;
    studentRoleByUid[studentUid] = copiedRole;
    roleResults[studentUid]?.push(`คุณติดตาม ${pub.players?.[targetId]?.name || 'ผู้เล่น'} และคัดลอกบท ${copiedRole}`);
  });

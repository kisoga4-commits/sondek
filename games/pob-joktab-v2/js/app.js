...
const ROLES = {
  ...
  doctor: { label: 'หมอ', icon: '💉', desc: 'ชุบชีวิตคนตายได้ 1 ครั้งต่อเกม' },
  student: { label: 'นักเรียน', icon: '📘', desc: 'ติดตามผู้เล่น 1 คนเพื่อคัดลอกบทบาท' },
...
const ROLE_UI_TEXT = {
  ...
  doctor: () => 'คุณคือหมอ ชุบชีวิตคนตายได้ 1 ครั้งต่อเกม คืนที่ชุบ ผู้ถูกชุบยังเล่น action ไม่ได้',
  student: () => 'คุณคือนักเรียน เลือกติดตามผู้เล่น 1 คน แล้วคุณจะกลายเป็นบทนั้นถาวร',
...
const ROLE_ACTION_CONFIG = {
  ...
  doctor: { requiresTarget: true, allowSelfTarget: false, actionLabel: 'ชุบชีวิต' },
  student: { requiresTarget: true, allowSelfTarget: false, actionLabel: 'ติดตาม' },
...
function resolvePlayerName(player = {}, uid = '') {
  const candidates = [
    player?.name,
    player?.displayName,
    player?.studentName,
  ...
  return safeUid ? `UID-${safeUid.slice(0, 6)}` : 'ไม่ทราบชื่อ';
}

function getRoomEntries(playersMap = state.duelPlayers) {
  return Object.entries(playersMap || {})
    .map(([uid, raw]) => {
      ...
      return {
        ...(raw || {}),
        uid: normalizedUid,
        name: resolvePlayerName(raw || {}, normalizedUid),
      };
    })
    .filter(Boolean);
}

function pobCountForPlayers(count) {
  return Math.max(1, Math.floor(Number(count || 0) / 4));
}

function rolePool(count) {
  const totalPlayers = Math.max(4, Math.min(24, Number(count || 4)));
  const pobCount = pobCountForPlayers(totalPlayers);
  const guaranteed = Array(pobCount).fill('pob');
  const randomPool = [];

  const pushUniqueGuaranteed = (roles) => {
    roles.forEach((role) => {
      if (!guaranteed.includes(role)) guaranteed.push(role);
    });
  };

  if (totalPlayers <= 7) {
    pushUniqueGuaranteed(['madman', 'hunter']);
    randomPool.push('villager', 'shaman', 'monk', 'police', 'doctor', 'student');
  } else if (totalPlayers <= 11) {
    pushUniqueGuaranteed(['madman', 'hunter', 'shaman']);
    randomPool.push('villager', 'monk', 'police', 'doctor', 'student');
  } else if (totalPlayers <= 15) {
    pushUniqueGuaranteed(['madman', 'hunter', 'shaman', 'police']);
    randomPool.push('villager', 'monk', 'doctor', 'student');
  } else if (totalPlayers <= 19) {
    pushUniqueGuaranteed(['madman', 'hunter', 'shaman', 'police', 'monk']);
    randomPool.push('villager', 'doctor', 'student');
  } else {
    pushUniqueGuaranteed(['madman', 'hunter', 'shaman', 'police', 'monk', 'doctor']);
    randomPool.push('villager', 'student');
  }
  ...
  const roles = [...guaranteed];
  const countByRole = roles.reduce((acc, role) => ({ ...acc, [role]: (acc[role] || 0) + 1 }), {});
  while (roles.length < totalPlayers) {
    const draw = randomPool[Math.floor(Math.random() * randomPool.length)] || 'villager';
    const limit = uniqueLimit[draw] || Number.POSITIVE_INFINITY;
    if ((countByRole[draw] || 0) >= limit) continue;
    roles.push(draw);
    countByRole[draw] = (countByRole[draw] || 0) + 1;
  }
  return roles;
}
...
function buildRandomizedGameState(entries, hostUid) {
  ...
  entries.forEach((p, idx) => {
    playersMap[p.uid] = { uid: p.uid, name: resolvePlayerName(p, p.uid), alive: true, roleHint: 'hidden', deathCause: '' };
  ...
  entries.forEach((p) => {
    ...
    privateMap[p.uid].partners = entries
      ...
      .map((x) => resolvePlayerName(x, x.uid));
  });
...
function currentNightActionStatusHtml() {
  ...
  const targetName = targetId ? (state.publicState?.players?.[targetId]?.name || targetId || 'ไม่ทราบชื่อ') : '';
...
function nightPhaseProgressHtml() {
  ...
  const waitingNames = pending.map((p) => resolvePlayerName(p, p.uid)).join(', ');
...
async function submitNightAction(targetId, acted = true) {
  ...
  const targetPlayer = normalizedTarget ? state.publicState?.players?.[normalizedTarget] : null;
  if (normalizedTarget && myRole === 'doctor' && targetPlayer?.alive) {
    openPopup({ title: 'เป้าหมายยังไม่ตาย', message: 'หมอชุบชีวิตได้เฉพาะผู้เล่นที่ตายแล้วเท่านั้น' });
    return;
  }
  if (normalizedTarget && myRole !== 'doctor' && !targetPlayer?.alive) {
...
async function resolveMorningWithPrivate(privateSnapshot) {
  ...
  await tx(paths.privateAll, (data) => {
    ...
    const studentRoleByUid = resolvedNight?.studentRoleByUid || {};
    const doctorUsedByUid = resolvedNight?.doctorUsedByUid || {};
    Object.keys(next).forEach((uid) => {
      ...
      const copiedRole = String(studentRoleByUid?.[uid] || '');
      const nextRole = copiedRole || role;
      ...
      next[uid] = {
        ...next[uid],
        role: nextRole,
        copiedRole: copiedRole || next?.[uid]?.copiedRole || '',
        doctorUsed: Boolean(next?.[uid]?.doctorUsed || doctorUsedByUid?.[uid]),
        nightAction: null,
        voteTarget: null,
    ...
    const pobPlayers = Object.keys(next).filter((uid) => String(next?.[uid]?.role || '') === 'pob');
    pobPlayers.forEach((uid) => {
      next[uid] = {
        ...next[uid],
        partners: pobPlayers
          .filter((pid) => pid !== uid)
          .map((pid) => state.publicState?.players?.[pid]?.name || pid || 'ไม่ทราบชื่อ'),
      };
    });
    return next;
  });
...
function renderNight() {
  ...
  const deadPlayers = Object.values(state.publicState?.players || {}).filter((p) => !p.alive);
  ...
  if (me?.alive && state.roleSheetRevealed) {
    if (iAmJailed) {
      ...
      `;
    } else if (!actionConfig.requiresTarget) {
      actionHtml = `
        <div class="tag">บทบาทนี้ไม่ต้องเลือกเป้าหมาย กดยืนยันเพื่อจบรอบกลางคืน</div>
        <button id="confirmNightActionBtn" class="btn big-btn" ${(acted || state.isSubmittingNightAction) ? 'disabled' : ''}>ยืนยันใช้สกิล</button>
      `;
    } else {
      const selectableTargets = myRole === 'doctor' ? deadPlayers : others;
      actionHtml = `
        <div class="big-grid">${selectableTargets.map((p) => `<button class="btn big-btn targetNight ${selectedTarget === p.uid ? 'ready' : ''}" data-id="${p.uid}" ${(acted || state.isSubmittingNightAction) ? 'disabled' : ''}>${p.name}${selectedTarget === p.uid ? ' ✅' : ''}</button>`).join('') || '<div class="tag">ไม่มีเป้าหมายที่เลือกได้</div>'}</div>
        <div class="tag" style="margin-top:.5rem;">เป้าหมายที่เลือก: ${selectedTarget ? (state.publicState?.players?.[selectedTarget]?.name || selectedTarget || 'ไม่ทราบชื่อ') : 'ยังไม่ได้เลือก'}</div>
  ...
  document.getElementById('confirmNightActionBtn')?.addEventListener('click', () => {
    const target = myRole === 'villager'
      ? state.uid
      : (actionConfig.requiresTarget ? state.selectedNightTargetId : '');
...
async function finalizeVoteByHost() {
  ...
  await tx(paths.public, (data) => {
    ...
    return {
      ...
      voteResolveLock: null,
      lastLogs: outUid
        ? [`${voteResult.players?.[outUid]?.name || outUid || 'ไม่ทราบชื่อ'} โดนขับไล่ออกจากหมู่บ้าน (ตาย)`]
...
function renderEnd() {
  ...
  els.end.innerHTML = `
    ...
    ${phaseMetaHtml()}
    <div class="tag">${winner === 'villager'
      ? '🎉 ฝั่งชาวบ้านชนะ'
      : (winner === 'madman'
        ? '🤪 คนบ้าชนะทันทีจากการโดนโหวต'
        : (winner === 'cancelled' ? '⛔ เกมยุติ' : '👹 ฝั่งปอบชนะ'))}</div>
  ...
  document.getElementById('restartGame')?.addEventListener('click', async () => {
    ...
    try {
      const entries = getRoomEntries();
      if (entries.length < 4 || entries.length > 24) {
...
async function initCloud() {
  ...
  onValue(paths.duelRoom, (snap) => {
    ...
    if (mode === 'pob_v2' && status === 'playing' && state.isHost && !state.publicState) {
      const entries = getRoomEntries();
      if (entries.length >= 4 && entries.length <= 24) {

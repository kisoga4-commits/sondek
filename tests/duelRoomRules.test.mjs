...
test('mode-specific max players are applied including logic_spy', () => {
  ...
  assert.equal(getRoomMaxPlayers('logic_spy'), 12);
});
...
test('start player cap is mode-specific and keeps core mode behavior', () => {
  ...
  assert.equal(getStartPlayerCap({ gameMode: 'logic_spy' }), 12);

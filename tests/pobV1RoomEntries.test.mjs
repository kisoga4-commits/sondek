...
function loadHelpers() {
  ...
  const candidateMatch = appSource.match(/function buildLegacyUidCandidates\(players = {}\) {[\s\S]*?\n}\n/);

  assert.ok(resolveMatch, 'ไม่พบฟังก์ชัน resolvePlayerName');
  assert.ok(entriesMatch, 'ไม่พบฟังก์ชัน getRoomEntries');
  assert.ok(candidateMatch, 'ไม่พบฟังก์ชัน buildLegacyUidCandidates');

  const script = new vm.Script(`${resolveMatch[0]}\n${entriesMatch[0]}\n${candidateMatch[0]}\n({ resolvePlayerName, getRoomEntries, buildLegacyUidCandidates });`);
  return script.runInNewContext({
    state: { duelPlayers: {}, uid: 'new_uid' },
    requestedUid: 'old_uid',
    readCachedRoomUid: () => 'cached_uid',
    Object,
    String,
    Set,
    Boolean,
...
test('buildLegacyUidCandidates รวม uid จาก query/cache และคัดเฉพาะคนที่อยู่ในห้อง', () => {
  const { buildLegacyUidCandidates } = loadHelpers();
  const candidates = buildLegacyUidCandidates({
    old_uid: { name: 'A' },
    cached_uid: { name: 'B' },
    stranger_uid: { name: 'C' },
  });
  assert.deepEqual(Array.from(candidates), ['old_uid', 'cached_uid']);
});

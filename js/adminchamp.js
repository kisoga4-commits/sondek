  ...
  courses.forEach((course) => {
    ...
    tr.innerHTML = `
      ...
      <td class="table-actions">
        ...
        <button class="btn btn-tiny btn-primary" data-act="copy" data-url="${course.quizLink || `${window.location.origin}/quiz.html?id=${course.courseId}`}">ลิงก์แบบทดสอบ</button>
      </td>`;
...
(async function ensureDefaultCourse() {
  ...
  await saveCourse({
    ...
    status: 'open',
    enrollmentUrl: '',
    quizLink: `${window.location.origin}/quiz.html?id=course_01`,
  });
})();
